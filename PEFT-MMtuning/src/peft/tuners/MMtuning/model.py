...
from .config import MMtuningConfig
from .gptq import QuantLinear
from .layer import Conv2d, Embedding, Linear, MMtuningLayer
...
-
    ...
    **Attributes**:
    ...
    prefix: str = "MMtuning_"

    def __init__(self, model, config, adapter_name) -> None:
        super().__init__(model, config, adapter_name)

    def _check_new_adapter_config(self, config: MMtuningConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        lora_config,
        adapter_name,
        target, # 要替换或修改的目标神经网络层。
        target_name, # 目标神经网络层的名称。
        parent,
        current_key, # 用于在层名称中匹配模式的键
        **optional_kwargs,
    ):
        if current_key is None:
            raise ValueError("Current Key shouldn't be `None`")
        # Regexp matching - Find key which matches current target_name in patterns provided
        # 这两行代码的作用是在一组模式键中找到与当前键（current_key）匹配的目标键（target_name_key）

        ## chain(lora_config.rank_pattern.keys(), lora_config.alpha_pattern.keys()): 使用 chain 函数将两组键串联成一个列表。
        # 要替换的module的名字
        pattern_keys = list(chain(lora_config.rank_pattern.keys(), lora_config.alpha_pattern.keys())) 

        ## filter() 函数的工作方式是对 iterable 中的每个元素应用 function 函数，如果结果为 True，则将该元素包含在结果迭代器中
        ## 遍历 pattern_keys 列表，筛选出与 current_key 匹配的键。
        target_name_key = next(filter(lambda key: re.match(f".*\.{key}$", current_key), pattern_keys), current_key)

        ## target_name_key: 是我们希望在 lora_config.rank_pattern 字典中查找的键。lora_config.r: 找不到时候返回的默认值
        ...
        if isinstance(target, Conv2d):
            ...
            new_module = self._create_new_module(lora_config, adapter_name, target, **kwargs) # 下面，用于autogpt的
            if adapter_name != self.active_adapter:
                # adding an additional adapter: it is not automatically trainable
                new_module.requires_grad_(False)
            self._replace_module(parent, target_name, new_module, target) # 在下面有介绍
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in self.model.modules():
                    if isinstance(m, MMtuningLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name):
        for module in self.model.modules():
            if isinstance(module, MMtuningLayer):
                if module.merged:
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, MMtuningLayer):
